...
public class JsonHelpers
{
  public static JsonSerializerOptions Options = new JsonSerializerOptions
  {
    ...
    Converters = { new NumericConverterFactory() },
  };
  ...
  public class SecureJsonTypeInfoModifier
  {
    ...
    public SecureJsonTypeInfoModifier(
      Func<string, string?> encryptionFunction,
      Func<string, string?> decryptionFunction
    )
    {
      _attributeConverterFactory = new SecureAttributeConverterFactory(
        encryptionFunction,
        decryptionFunction
      );
    }
    ...
    /// </param>
    public SecureJsonTypeInfoModifier(
      SecureAttributeConverterFactory secureAttributeConverterFactory
    )
    {
    ...
    public void Modify(JsonTypeInfo typeInfo)
    {
      ...
      foreach (JsonPropertyInfo propertyInfo in typeInfo.Properties)
      {
        ...
        var secureAttributes =
          propertyInfo.AttributeProvider?.GetCustomAttributes(typeof(SecureAttribute), true)
          ?? Array.Empty<object>();
  ...
  public class SecureAttributeConverterFactory : JsonConverterFactory
  {
    ...
    /// <param name="encryptionFunction">Function that takes in the plain text and encrypts the value.</param>
    /// <param name="decryptionFunction">Function that decrypts the JSON value.</param>
    public SecureAttributeConverterFactory(
      Func<string, string?> encryptionFunction,
      Func<string, string?> decryptionFunction
    )
    {
    ...
    )
    {
      ...
      return converterInstance as JsonConverter
        ?? throw new JsonException(
          $"Converter creation failed for type '{typeToConvert.FullName}'."
        );
    }
  }
  ...
  private class SecureAttributeConverter<T> : JsonConverter<T>
  {
    ...
    public SecureAttributeConverter(
      Func<string, string?> encryptionFunction,
      Func<string, string?> decryptionFunction
    )
    {
    ...
    )
    {
      string decryptedValue =
        _decryptionFunction(reader.GetString() ?? throw new NullReferenceException())
        ?? string.Empty;
    ...
    public override void Write(Utf8JsonWriter writer, T value, JsonSerializerOptions options)
    {
      string encryptedValue =
        _encryptionFunction(value?.ToString() ?? string.Empty) ?? string.Empty;
  ...
  private class NumericConverter<T> : JsonConverter<T>
  {
    ...
    )
    {
      ...
      if (
        ...
        && double.TryParse(reader.GetString(), out var doubleValue)
      )
      {
