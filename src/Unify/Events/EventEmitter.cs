...
public partial class EventEmitter : IEventEmitter
{
  ...
  /// </summary>
  private readonly Dictionary<string, List<IEventEmitterListener>> _listeners =
    new Dictionary<string, List<IEventEmitterListener>>();
  ...
  private void AddEventListener(
    string eventName,
    ICallback callback,
    bool oneTimeListener = false,
    bool prependListener = false
  )
  {
    ...
    UnifyRuntime.Current.RuntimeLog.Debug(
      $"Added event listener `{eventEmitterListener}` to `{eventName}`"
    );
  }
  ...
  public void Emit(string eventName, params object?[]? parameters)
  {
    ...
    if (_listeners.TryGetValue(eventName, out List<IEventEmitterListener>? value))
    {
      ...
      UnifyRuntime.Current.RuntimeLog.Debug(
        "EventEmitter",
        $"Event \"{eventName}\" has been fired."
      );
  ...
  // Adding
  public void AddListener(string eventName, ICallback callback) =>
    AddEventListener(eventName, callback);

  public void On(string eventName, ICallback callback) => AddEventListener(eventName, callback);

  public void Once(string eventName, ICallback callback) =>
    AddEventListener(eventName, callback, true);

  public void PrependListener(string eventName, ICallback callback) =>
    AddEventListener(eventName, callback, false, true);

  public void PrependOnceListener(string eventName, ICallback callback) =>
    AddEventListener(eventName, callback, true, true);
  ...
  public void RemoveListener(string eventName, ICallback callback)
  {
    ...
    if (removedCount == 0)
    {
      UnifyRuntime.Current.RuntimeLog.Verbose(
        $"Attempt to remove callback not in the `{eventName}` event."
      );
  ...
  public void RemoveAllListeners(string eventName) =>
    _listeners.Remove(NormalizeEventName(eventName));
